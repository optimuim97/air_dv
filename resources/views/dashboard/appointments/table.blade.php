<div class="card-body p-0">
    <div class="table-responsive">
        <table class="table" id="appointments-table">
            <thead>
            <tr>
                <th>Creator Id</th>
                <th>Other Id</th>
                <th>Date</th>
                <th>Hour</th>
                <th>Date Time</th>
                <th>Is Confirmed</th>
                <th>Is Report</th>
                <th>Report Date</th>
                <th colspan="3">Action</th>
            </tr>
            </thead>
            <tbody>
            @foreach($appointments as $appointment)
                <tr>
                    <td>{{ $appointment->creator_id }}</td>
                    <td>{{ $appointment->other_id }}</td>
                    <td>{{ $appointment->date }}</td>
                    <td>{{ $appointment->hour }}</td>
                    <td>{{ $appointment->date_time }}</td>
                    <td>{{ $appointment->is_confirmed }}</td>
                    <td>{{ $appointment->is_report }}</td>
                    <td>{{ $appointment->report_date }}</td>
                    <td  style="width: 120px">
                        {!! Form::open(['route' => ['dashboard.appointments.destroy', $appointment->id], 'method' => 'delete']) !!}
                        <div class='btn-group'>
                            <a href="{{ route('dashboard.appointments.show', [$appointment->id]) }}"
                               class='btn btn-default btn-xs'>
                                <i class="far fa-eye"></i>
                            </a>
                            <a href="{{ route('dashboard.appointments.edit', [$appointment->id]) }}"
                               class='btn btn-default btn-xs'>
                                <i class="far fa-edit"></i>
                            </a>
                            {!! Form::button('<i class="far fa-trash-alt"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-xs', 'onclick' => "return confirm('Are you sure?')"]) !!}
                        </div>
                        {!! Form::close() !!}
                    </td>
                </tr>
            @endforeach
            </tbody>
        </table>
    </div>

    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $appointments])
        </div>
    </div>
</div>
